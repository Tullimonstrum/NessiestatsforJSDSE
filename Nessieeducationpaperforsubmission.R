...
barplot(height = temp$counts, width = 1, space = NULL, xlab = "Interpreted Beaufort Wind State", names.arg = seq(0, 9), ylab = "Frequency", col = "white", main = "c)")
...
##Figure 2
boxplot (AllDistNADpriority ~ Handedness, data=Nessie, range=0)
...
modelLength <- glm(SeenLengthSI ~ Report.Year.for.analysis + Handedness + Location + Anonymous.at.point.of.interview, data = Nessie, family = Gamma)
Anova(modelLength)
plot (modelLength)
...
modelLength <- glm(SeenLengthSI ~ Report.Year.for.analysis, data = Nessie[Nessie$NessieSeenLengthSI<20,], family=Gamma)
Anova(modelLength)
plot (modelLength)
